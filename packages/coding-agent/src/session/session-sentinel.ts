 ...
 */
export async function recordAbnormalSessionExit(sessionManager: SessionManager): Promise<boolean> {
...
export async function runSessionExitSentinel(options: SessionExitSentinelOptions): Promise<boolean> {
	...
	try {
		return await recordAbnormalSessionExit(sessionManager);
	} finally {
		await sessionManager.flush();
	}
}
...
export function resolveSentinelSpawnCmd(workerArg = SESSION_SENTINEL_WORKER_ARG): string[] {
	const cliPath = path.resolve(import.meta.dir, "..", "cli.ts");
	return [process.execPath, cliPath, workerArg];
}

export function startSessionExitSentinel(sessionFile: string): (() => void) | undefined {
	const cmd = resolveSentinelSpawnCmd();
	try {
