...
- Added `omp pair begin|approve|deny|claim|list|revoke|rotate` for foreground, capability-scoped native broker enrollment with one-time QR URIs and redacted device state ([#13](https://github.com/n24q02m/oh-my-pi/issues/13)).
...
- Bounded collaboration relay reconnect attempts so persistent transient failures terminate cleanly instead of retrying indefinitely ([#3](https://github.com/n24q02m/oh-my-pi/issues/3)).
