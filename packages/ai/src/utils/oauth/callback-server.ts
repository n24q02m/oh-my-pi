	...
	try {
		...
		for (const name in interfaces) {
			...
			for (const entry of entries) {
				...
				if (isIpv6 && entry.internal && entry.address === IPV6_LOOPBACK) {
					return true;
				}
			}
		}
...
function isAddressInUse(error: unknown): boolean {
	...
	if ("message" in error && typeof error.message === "string") {
		return /EADDRINUSE|in use/i.test(error.message);
	}
	return false;
}
...
export interface OAuthCallbackFlowOptions {
	 ...
	 * With fallback disabled, {@link OAuthCallbackFlow.login} throws an Error
	 * immediately so the caller can surface an actionable message before
	 * opening the browser.
	 */
...
export abstract class OAuthCallbackFlow {
	...
	#loginCancelledError(): Error {
		return new Error(`OAuth callback cancelled: ${this.ctrl.signal?.reason}`);
	}
	...
	#createServer(port: number, expectedState: string): CallbackServer {
		if (this.callbackHostname !== DEFAULT_HOSTNAME || !ipv6LoopbackAvailable()) {
			return this.#serve(
				this.callbackHostname === DEFAULT_HOSTNAME ? IPV4_LOOPBACK : this.callbackHostname,
				port,
				expectedState,
			);
		}
	...
	#waitForCallback(expectedState: string): Promise<CallbackResult> {
		...
		if (this.ctrl.onManualCodeInput) {
			...
			const manualPromise = (async (): Promise<CallbackResult> => {
				...
				throw this.#loginCancelledError();
			})();
